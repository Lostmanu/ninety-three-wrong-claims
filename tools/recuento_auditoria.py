...
# PUBLICACIÓN (2026-09-12): el único cambio respecto al fichero que corrió en el laboratorio es que
# la ruta del documento ya no se calcula subiendo cuatro directorios desde __file__, sino que es un
# argumento con este valor por defecto. Lo demás va byte a byte como estaba, incluidos los comentarios
# que explican los dos fallos que este generador existe para impedir.
DOC_POR_DEFECTO = os.path.join(os.path.dirname(os.path.dirname(_AQUI)),
                               "register", "AUDITORIA_DEL_METODO.md")
...
def main():
    ...
    ap.add_argument("--doc", default=DOC_POR_DEFECTO, help="ruta del registro (por defecto: register/AUDITORIA_DEL_METODO.md)")
    args = ap.parse_args()
    DOC = args.doc
    try:
